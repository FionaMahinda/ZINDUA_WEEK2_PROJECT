...
WHERE 
    ...
    UPDATE financial_details
SET fee_cleared = IF(fee_paid >= sem_fee, 1, 0);
...
SELECT 
    SUM(CASE WHEN cd.next_of_kin_relation = 'Mother' THEN 1 ELSE 0 END) * 100 / COUNT(*) AS mother_percentage,
    SUM(CASE WHEN cd.next_of_kin_relation = 'Father' THEN 1 ELSE 0 END) * 100 / COUNT(*) AS father_percentage
FROM 
    contact_details cd;
